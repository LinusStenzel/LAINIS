...
public class HandOutcome {
	...
	private void refine() {
		...
		case STRAIGHT:
			...
			if (topValueStraight == Value.ACE
					&& topCards.get(0).getValue() == Value.TWO) {
		...
		case STRAIGHT_FLUSH:
			...
			// straight may go from ace to five
			if (topValueStaightFlush == Value.ACE
					&& topCards.get(0).getValue() == Value.TWO) {
	...
	private void holeCardsInTopCards() {
		for (int i = 0; i < topCards.size(); i++) {
			if (topCards.get(i).equals(holeCards[0])
					|| topCards.get(i).equals(holeCards[1])) {
				holeCardsInTopCards++;
			}
		}
	}
