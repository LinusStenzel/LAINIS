...
public class BettingRound {
	...
	public void addAction(Who who, String action) {
		...
		case util.CHECK:
			a = new Action();
			a.setActionType(ActionType.CHECK);
			a.setWho(who);
			a.setAmount(0);
			break;
		case util.CALL:
		...
		case util.FOLD:
			a = new Action();
			a.setActionType(ActionType.FOLD);
			a.setWho(who);
			a.setAmount(0);
			break;
		default: // = SB or BB
			...
			if (action.startsWith(util.SMALL_BLIND)) {
				...
				a = new Action();
				a.setActionType(ActionType.SB);
				a.setWho(who);
				a.setAmount(blindAmount);
	...
	/**
	 * converts this betting round into a 6 or 7 dimensional vector by looping over
	 * the actions and converting them into a six dimensional vector space for four
